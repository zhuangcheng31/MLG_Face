...
def plot_landmark_compare(landmark, img, img_gt):
    ...
    a=all_landmarks[:, 0]
    b=all_landmarks[:, 1]
    plt.scatter(all_landmarks[:, 0], all_landmarks[:, 1], linewidths=0.5, c='red')  #（272，2，0.5） 绘制散点图
    plt.axis('off')
    return fig_withlm

def merge_and_scale_heatmap(heatmap, scale):
    merged = np.mean(heatmap, axis=0) #    merged = np.mean(heatmap, axis=0)
    h, w = merged.shape  #chenggai  本来是：h, w = merged.shape
