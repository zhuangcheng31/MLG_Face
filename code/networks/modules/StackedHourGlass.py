...
from code.networks.modules.DeformConv import DeformConv2d
...
class FeedbackHourGlass(nn.Module):
    def __init__(self, num_feature, num_keypoints):
        ...
        self.compress_in = nn.Conv2d(2 * self.num_feature, 2 * self.num_feature, 1)   #self.num_feature =256
        
        self.hg = HourGlass(4, 2 * self.num_feature, False)
        self.hg_conv_out = nn.Sequential(
            ResidualBlock(self.num_feature, self.num_feature, False),    # 源码 cheng先注释  改为下一行的可变形卷积试试
            # DeformConv2d(self.num_feature, self.num_feature),  # chengjia

            Lin(self.num_feature, self.num_feature, False),
            nn.Conv2d(self.num_feature, self.num_keypoints, 1))

    def forward(self, x, last_hidden=None):   # x        ([4, 3, 128, 128])
        feature = self.pre_conv_block(x)      #feature = ([4, 256, 32, 32])
        if last_hidden is None:
...
#=============================================================================================
        heatmap = self.hg_conv_out(feature[:, :self.num_feature])
        return heatmap, feature[:, self.num_feature:]
...
class Lin(nn.Module):
    def __init__(self,numIn,numout,need_bn=True):
        ...
        if need_bn:
            ...
            self.conv_block = nn.Sequential(
                nn.Conv2d(numIn,numout,1),   #256，256，1
                nn.ReLU(inplace=True)
            )
    def forward(self,x):
