...
class FeedbackBlock_2(nn.Module):
    def __init__(self, num_features, num_groups, upscale_factor, act_type, norm_type, device=torch.device('cuda')):
        ...
        self.compress_out = ConvBlock(num_groups*num_features, num_features,   #num_groups*num_features, num_features,  #0317
                                      kernel_size=1,
                                      act_type=act_type, norm_type=norm_type)
...
class FeedbackBlock(nn.Module):
    def __init__(self, num_features, num_groups, upscale_factor, act_type, norm_type, device=torch.device('cuda')):
        ...
        self.compress_in = ConvBlock(2*num_features, num_features,     # 2*num_features
                                     kernel_size=1,
                                     act_type=act_type, norm_type=norm_type)
    ...
    def forward(self, x):
        ...
        for idx in range(self.num_groups):
            LD_L = torch.cat(tuple(lr_features), 1)
            if idx > 0:
                LD_L = self.uptranBlocks_s[idx-1](LD_L)
            LD_H = self.upBlocks_s[idx](LD_L)

            hr_features.append(LD_H)

            LD_H = torch.cat(tuple(hr_features), 1)
            if idx > 0:
                LD_H = self.downtranBlocks_s[idx-1](LD_H)
            LD_L = self.downBlocks_s[idx](LD_H)
...
class SRFBN(nn.Module):
    def __init__(self, in_channels, out_channels, num_features, num_steps, num_groups, upscale_factor, act_type='prelu', norm_type=None, device=torch.device('cuda')):
        ...
        # basic block
        self.block = FeedbackBlock(num_features, num_groups, upscale_factor, act_type, norm_type, device=device)
